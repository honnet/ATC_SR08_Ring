...
//#define SUOTAR_ALLOW_BANK_SELECTION
#endif
...
int app_read_image_headers(uint8_t image_bank, uint8_t* data, uint32_t data_len)
{
    ...
    suota_state.mem_base_add = 0x18000;
    suota_state.suota_image_len = pfwHeader->all_size + sizeof(image_header_t);
    // verify image positions
    // the code size must be less than the space between images
    if (!pfwHeader->all_size || ((pfwHeader->all_size + sizeof(image_header_t)) > 0x14000))
    {
    ...
    pImageHeader.validflag = 0xff;
    pImageHeader.imageid = pfwHeader->imageid;
    pImageHeader.all_size= pfwHeader->all_size;
    ...
    memcpy(pImageHeader.reserved,pfwHeader->reserved, sizeof(pfwHeader->reserved));
